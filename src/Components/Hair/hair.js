...
const Hair = () => {
    ...
    useEffect(() => {
        const getData = async () => {
            const response = await fetch(`${Url}/get_product_by_cat/4/`)
        ...
        <div dir="rtl" className="w-full flex sm:mb-44 justify-start font-KALAMEHBOLD mt-44 sm:mt-5 h-fit">
            <div className="w-full sm:flex sm:flex-col sm:items-center  h-fit">
                <div className="w-full flex justify-between  relative items-center ">
                    <h1 className='mr-20 sm:mr-5 font-bold text-xl '>خدمات رنگ و مو</h1>
                    <Link to={`/services/${4}`}><h1 className='ml-20 sm:ml-5 font-bold  text-xl'>بیشتر</h1></Link>
                </div>
